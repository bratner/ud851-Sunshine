...
//      DONE (6) Move the cursor to the appropriate position
        ...
        float highTemp = mCursor.getFloat(MainActivity.INDEX_WEATHER_MAX_TEMP);
        float lowTemp = mCursor.getFloat(MainActivity.INDEX_WEATHER_MIN_TEMP);
        String highAndLowTemps = SunshineWeatherUtils.formatHighLows(mContext, highTemp, lowTemp);
//      DONE (7) Generate a weather summary with the date, description, high and low
        String weatherForThisDay = dateString + " - " + description + " - " + highAndLowTemps;
